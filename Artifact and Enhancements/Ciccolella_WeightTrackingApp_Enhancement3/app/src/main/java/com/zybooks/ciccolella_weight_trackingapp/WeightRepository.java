...
public class WeightRepository {
     ...
     */
    public void addOrUpdateWeight(String username, String date, double weightValue, String noteText) {
        ...
        values.put(WeightTrackerDbHelper.COLUMN_WEIGHT_NOTE, noteText);
     ...
     */
    public void updateWeightById(long id, String username, String date, double weightValue, String noteText) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
    ...
    public void addOrUpdateNote(long weightEntryId, String noteContent) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();

        ContentValues values = new ContentValues();
        values.put(WeightTrackerDbHelper.COLUMN_WEIGHT_NOTE, noteContent);

        String selection = WeightTrackerDbHelper.COLUMN_WEIGHT_ID + "=?";
        String[] selectionArgs = { String.valueOf(weightEntryId) };
        ...
        if (rowsUpdated == 0) {
            // Insert if the note doesn't exist (shouldn't happen here because we are updating existing rows)
            values.put(WeightTrackerDbHelper.COLUMN_WEIGHT_ID, weightEntryId);
            db.insert(WeightTrackerDbHelper.TABLE_WEIGHTS, null, values);
        }
    }

    public String getNoteForWeightEntry(long weightEntryId) {
        SQLiteDatabase db = dbHelper.getReadableDatabase();

        String[] projection = {
                WeightTrackerDbHelper.COLUMN_WEIGHT_NOTE
        };
        ...
        if (cursor != null && cursor.moveToFirst()) {
            note = cursor.getString(cursor.getColumnIndexOrThrow(WeightTrackerDbHelper.COLUMN_WEIGHT_NOTE));
            cursor.close();
        }
